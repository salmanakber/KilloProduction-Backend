...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const imageFile of imageFiles) {
      if (imageFile && imageFile.size > 0) {
        try {
          ...
          if (imageFile.size > maxSize) continue
          ...
          console.error("Quote image upload error:", uploadError)
        }
      }
    }
    ...
    console.error("Quote image upload route error:", error)
