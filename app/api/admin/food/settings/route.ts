...
// GET - Fetch food module settings
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      // If FoodSettings doesn't exist yet, use defaults (will be created on first save)
...
// PUT - Update food module settings
export async function PUT(request: NextRequest) {
  try {
    ...
    // Upsert food settings (create if doesn't exist, update if exists)
    ...
    return NextResponse.json({ error: "Failed to update food settings", details: error.message }, { status: 500 })
  }
}
