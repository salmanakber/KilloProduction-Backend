...
import type { Payment, Prisma, User } from "@prisma/client"
import Stripe from "stripe"
import { getGlobalSocketServer } from "@/lib/socket-server"
import { issueRideStartOtp } from "@/lib/ride-start-otp"

export function getRefundMeta(meta: Prisma.JsonValue | null): Record<string, unknown> | null {
...
export type RefundApprovalOk = {
  ok: true
  shouldDeferSettlement: boolean
  txResult: string
  refundAmount: number
  refundCourierBookingId: string | null
}

/**
 * Full admin APPROVE flow (wallet / Stripe / defer courier pickup).
 * Caller must ensure refund metadata exists and status is PENDING.
 */
export async function executeRefundApproval(opts: {
  payment: Payment & { user: User | null }
  processedBy: string
  adminNote: string
}): Promise<RefundApprovalOk | { ok: false; error: string }> {
  const { payment, processedBy, adminNote } = opts

  try {
    const refund = getRefundMeta(payment.metadata)
    if (!refund) return { ok: false, error: "Refund request not found on payment" }
    if (String(refund.status || "PENDING") !== "PENDING") {
      return { ok: false, error: "Refund already processed" }
    }
          ...
          if (pickupLatitude && pickupLongitude) {
            ...
            if (socketServer) {
              await socketServer.broadcastCourierNewRequestToRiders({
                ...
                pickupLatitude: refundBooking.pickupLatitude,
                pickupLongitude: refundBooking.pickupLongitude,
    ...
    console.error("executeRefundApproval:", e)
    return { ok: false, error: e instanceof Error ? e.message : "Refund approval failed" }
  }
}
